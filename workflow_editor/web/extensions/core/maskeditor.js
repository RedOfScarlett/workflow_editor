...
import { DCCDialog, $el } from "../../scripts/ui.js";
import { DCCApp as DCCApp } from "../../scripts/app.js";
...
async function uploadMask(filepath, formData) {
	...
	DCCApp.clipspace.imgs[DCCApp.clipspace['selectedIndex']] = new Image();
	DCCApp.clipspace.imgs[DCCApp.clipspace['selectedIndex']].src = api.apiURL("/view?" + new URLSearchParams(filepath).toString() + app.getPreviewFormatParam() + app.getRandParam());

	if(DCCApp.clipspace.images)
		DCCApp.clipspace.images[DCCApp.clipspace['selectedIndex']] = filepath;
...
class MaskEditorDialog extends DCCDialog {
	...
	createLeftSlider(self, name, callback) {
		...
		divElement.style.backgroundColor = "var(--dcc-input-bg)";
			...
			mutations.forEach(function(mutation) {
					if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
						if(self.last_display_style && self.last_display_style != 'none' && self.element.style.display == 'none') {
							...
							DCCApp.onClipspaceEditorClosed();
						}
	...
	async setImages(imgCanvas) {
		...
		// image load
		const filepath = DCCApp.clipspace.images;

		const alpha_url = new URL(DCCApp.clipspace.imgs[DCCApp.clipspace['selectedIndex']].src)
		...
		// original image load
		const rgb_url = new URL(DCCApp.clipspace.imgs[DCCApp.clipspace['selectedIndex']].src);
	...
	async save() {
		...
		if(DCCApp.clipspace.images)
			DCCApp.clipspace.images[0] = item;

		if(DCCApp.clipspace.widgets) {
			const index = DCCApp.clipspace.widgets.findIndex(obj => obj.name === 'image');

			if(index >= 0)
				DCCApp.clipspace.widgets[index].value = item;
		}
...
app.registerExtension({
	...
	init(app) {
		...
		const context_predicate = () => DCCApp.clipspace && DCCApp.clipspace.imgs && DCCApp.clipspace.imgs.length > 0
		ClipspaceDialog.registerButton("MaskEditor", context_predicate, DCCApp.open_maskeditor);
	}
});
