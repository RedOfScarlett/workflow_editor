	...
	"solarized": {
		...
		"colors": {
			...
			"dcc_base": {
				...
				"dcc-menu-bg": "#073642", // Base02
				"dcc-input-bg": "#002b36", // Base03
...
const id = "DCC.ColorPalette";
const idCustomColorPalettes = "DCC.CustomColorPalettes";
...
app.registerExtension({
	...
	addCustomNodeDefs(node_defs) {
		...
		const getColorPaletteTemplate = async () => {
			...
			for (const key in defaultColorPalette.colors.dcc_base) {
				if (!colorPalette.colors.dcc_base[key]) {
					colorPalette.colors.dcc_base[key] = "";
				}
			}
		...
		const loadColorPalette = async (colorPalette) => {
			...
			if (colorPalette.colors) {
				...
				// Sets the color of DCCUI elements
				if (colorPalette.colors.dcc_base) {
					const rootStyle = document.documentElement.style;
					for (const key in colorPalette.colors.dcc_base) {
						rootStyle.setProperty('--' + key, colorPalette.colors.dcc_base[key]);
					}
				}
