import { DCCLogging } from "./logging.js";
import { DCCWidgets as DCCWidgets } from "./widgets.js";
import { DCCUI, $el } from "./ui.js";
...
export const ANIM_PREVIEW_WIDGET = "$$dcc_animation_preview"
...
/**
 * @typedef {import("types/dcc").DCCExtension} DCCExtension
 */
...
export class DCCApp {
	...
	constructor() {
		this.ui = new DCCUI(this);
		this.logging = new DCCLogging(this);
	...
	getPreviewFormatParam() {
		let preview_format = this.ui.settings.getSettingValue("DCC.PreviewFormat");
		if(preview_format)
	...
	static onClipspaceEditorSave() {
		if(DCCApp.clipspace_return_node) {
			DCCApp.pasteFromClipspace(DCCApp.clipspace_return_node);
		}
	}

	static onClipspaceEditorClosed() {
		DCCApp.clipspace_return_node = null;
	}
	...
	static copyToClipspace(node) {
		...
		if(DCCApp.clipspace_invalidate_handler) {
			DCCApp.clipspace_invalidate_handler();
		}
	}
	...
	static pasteFromClipspace(node) {
		if(DCCApp.clipspace) {
			// image paste
			if(DCCApp.clipspace.imgs && node.imgs) {
				if(node.images && DCCApp.clipspace.images) {
					if(DCCApp.clipspace['img_paste_mode'] == 'selected') {
						node.images = [DCCApp.clipspace.images[DCCApp.clipspace['selectedIndex']]];
					}
					else {
						node.images = DCCApp.clipspace.images;
					}
				...
				if(DCCApp.clipspace.imgs) {
					// deep-copy to cut link with clipspace
					if(DCCApp.clipspace['img_paste_mode'] == 'selected') {
						const img = new Image();
						img.src = DCCApp.clipspace.imgs[DCCApp.clipspace['selectedIndex']].src;
						node.imgs = [img];
						node.imageIndex = 0;
					}
					else {
						const imgs = [];
						for(let i=0; i<DCCApp.clipspace.imgs.length; i++) {
							imgs[i] = new Image();
							imgs[i].src = DCCApp.clipspace.imgs[i].src;
							node.imgs = imgs;
						}
					}
				}
			...
			if(node.widgets) {
				if(DCCApp.clipspace.images) {
					const clip_image = DCCApp.clipspace.images[DCCApp.clipspace['selectedIndex']];
				...
				if(DCCApp.clipspace.widgets) {
					DCCApp.clipspace.widgets.forEach(({ type, name, value }) => {
	 ...
	 * @param {keyof DCCExtension} method The extension callback to execute
		...
		node.prototype.getExtraMenuOptions = function (_, options) {
			...
			if(!DCCApp.clipspace_return_node) {
				options.push({
						...
						callback: (obj) => { DCCApp.copyToClipspace(this); }
					});

				if(DCCApp.clipspace != null) {
					options.push({
							content: "Paste (Clipspace)",
							callback: () => { DCCApp.pasteFromClipspace(this); }
						});
				}

				if(DCCApp.isImageNode(this)) {
					options.push({
							content: "Open in MaskEditor",
							callback: (obj) => {
								DCCApp.copyToClipspace(this);
								DCCApp.clipspace_return_node = this;
								DCCApp.open_maskeditor();
							}
						});
				}
			}
			...
			for (const item of items) {
				if (item.type.startsWith('image/')) {
					...
					if (this.canvas.current_node &&
						...
						DCCApp.isImageNode(this.canvas.current_node)) {
	...
	//    this.logging.addEntry("DCC.App", "debug", { Extensions: extensions });
	//
	...
	async setup() {
		...
		function resizeCanvas() {
			// Limit minimal scale to 1, see https://github.com/dccanonymous/DCCUI/pull/845
	...
	//getWidgetType(inputData, inputIndex) {
	//	const type = inputData[0];
	//	if (Array.isArray(type)) {
	//		return "COMBO";
	//	} else if (`${type}:${inputIndex}` in this.widgets) {
	//		return `${type}:${inputIndex}`;
	...
	async registerNodeDef(nodeId, nodeData) {
		...
		const node = Object.assign(
			function DCCNode() {
				var inputs = nodeData["inputs"]["required"];
				if (nodeData["inputs"]["optional"] != undefined) {
					inputs = Object.assign({}, nodeData["inputs"]["required"], nodeData["inputs"]["optional"]);
				}
				for (const inputIndex in inputs) {
					const inputData = JSON.parse(inputs[inputIndex]);
					this.addInput(inputData["name"], inputData["type"]);
				}

				var attributes = nodeData["attributes"]["required"];
				if (nodeData["attributes"]["optional"] != undefined) {
					attributes = Object.assign({}, nodeData["attributes"]["required"], nodeData["attributes"]["optional"]);
				}
				const config = { minWidth: 1, minHeight: 1 };
				for (const attributeIndex in attributes) {
					const attributeData = JSON.parse(attributes[attributeIndex]);
					const attributeName = attributeData["name"];
					const widgetType = attributeData["type"];
					let defaultValue = "";
					let defaultCallBack = function () {};
					if (attributeName == "outputdatatype") {
						defaultValue = widgetType;
						defaultCallBack = function (value, widget, node) {
							const info = node.getOutputInfo(0);
							node.removeOutput(0);
							node.addOutput(info.name, value, { shape: LiteGraph.CIRCLE_SHAPE });
						}
						Object.assign(config, self.widgets['DATATYPE'](this, attributeName, defaultValue, defaultCallBack, app) || {});
					}
					else if (attributeName == "datafile") {
						defaultValue = widgetType;
						defaultCallBack = function (value, widget, node) {
							node.setOutputDataFormat(0, value);
						}
						Object.assign(config, self.widgets['DATAFILE'](this, attributeName, defaultValue, defaultCallBack, app) || {});
					}
					else {
						this.addProperty(attributeName, defaultValue, widgetType);
						Object.assign(config, self.widgets['STRING'](this, attributeName, defaultValue, defaultCallBack, app) || {});
					}
				}
				var outputs = nodeData["outputs"]
				for (const outputIndex in outputs) {
					const outputData = JSON.parse(outputs[outputIndex]);
					this.addOutput(outputData["name"], outputData["type"], { shape: LiteGraph.CIRCLE_SHAPE });
				}
			...
			},
			{
				...
				dccClass: nodeData.name,
				nodeData
			}
		);
		node.prototype.dccClass = nodeData.name;

		const onAdded = node.prototype.onAdded;
		node.prototype.onAdded = function (graph) {
			onAdded?.(this, arguments);
			if (this.outputs) {
				for (var i = 0; i < this.outputs.length; i++) {
					this.outputs[i].name = this.outputs[i].name + '_' + this.id
				}
			}
		}
		const onConnectInput = node.prototype.onConnectInput;
		node.prototype.onConnectInput = function (targetSlot, type, output, originNode, originSlot) {
			const v = onConnectInput?.(this, arguments);
			//console.log("originNode.outputs[originSlot].name", originNode.outputs[originSlot].name);
			this.inputs[targetSlot].name = originNode.outputs[originSlot].name;
			return v;
		};
	...
	showMissingNodesError(missingNodeTypes, hasAddedNodes = true) {
		...
		this.logging.addEntry("DCC.App", "warn", {
	...
	async executeWorkflow(number, batchCount = 1) {
		...
		try {
			while (this.#queueItems.length) {
				...
				for (let i = 0; i < batchCount; i++) {
					const p = await this.graphToWorkflow();

					try {
						const res = await api.executeWorkflow(number, p);
	...
	/**
	 * Registers a DCC web extension with the app
	 * @param {DCCExtension} extension
	 */
