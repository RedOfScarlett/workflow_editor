import { $el, DCCDialog } from "./ui.js";
...
class DCCLoggingDialog extends DCCDialog {
	constructor(logging) {
		super();
	...
	export() {
		...
		const a = $el("a", {
			href: url,
			download: `dccui-logs-${Date.now()}.json`,
			style: { display: "none" },
			parent: document.body,
		});
		a.click();
...
export class DCCLogging {
	...
	constructor(app) {
		...
		this.dialog = new DCCLoggingDialog(this);
		this.addSetting();
		this.catchUnhandled();
		this.addInitData();
	}
	...
	addSetting() {
		const settingId = "DCC.Logging.Enabled";
