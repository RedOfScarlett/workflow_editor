...
export function createImageHost(node) {
	const el = $el("div.dcc-img-preview");
	let currentImgs;
	let first = true;
	...
	function updateSize() {
		...
		if (currentImgs) {
			...
			if (first) {
				...
				el.style.setProperty("--dcc-widget-min-height", elH);
			} else {
				el.style.setProperty("--dcc-widget-min-height", null);
			}
			...
			el.style.setProperty("--dcc-img-preview-width", w);
			el.style.setProperty("--dcc-img-preview-height", h);
		}
	}
	return {
		el,
